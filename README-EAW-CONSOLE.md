...
## ○△□◇などの記号類
これらの記号類は大変難しい。
英語件の人々はTUIでこれらのAmbiguous文字を半角で利用します。
一方JIS X 0208由来のこれら文字を和文で半角とすることは耐え難い。
幸いUnicodeには多くの記号類が収録されているので、
TUIアプリケーションの開発者に丁寧に説明し、文字幅固定の文字に置き換えてもらうことで問題を回避できる。
たとえば、以下のように代替できる。
...
`[•]` U+2022 BULLETがEAW=Aであるのが厄介だ、しかしこれはJIS X 0208に存在しなかった文字なのでこのロケールでは半角とする。
これが受け入れられない場合、`[∙]` U+2219 BULLET OPERATORで代替すると良い。
