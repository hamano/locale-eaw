...
## 解決方法その0: 曖昧な文字を半角として生きていく(既定のロケール)
以下のような辛さがあります。
- ♨※などJIS X 0208由来の記号が半角
- Unicode収録絵文字やNerdFontの絵文字が半角になる
- 一般的な日本語フォントの文字幅と一致しない
- ⑳が半角で㉑が全角になる

## 解決方法その1: 曖昧な文字を全角に統一 EAW-FULLWIDTH

この問題の古典的な解決方法は曖昧な文字幅を全角に統一する事です。
...
すべての曖昧な文字を全角にすると以下のような問題が起こります。
...
ロケールで曖昧幅の文字を全角に変更しても、TUIアプリがロケールを参照せず表示が壊れてしまうことがあります。
TUIアプリで曖昧幅の文字の利用を避けてもらうのが一番良いのですが修正してもらうのはなかなか困難です。
そこで、TUIアプリが出力する曖昧幅文字を代替の半角文字に置換するtty proxyを作りました。
これにより、lazygitやAI CLI agentなどで表示が崩れないようにできます。
